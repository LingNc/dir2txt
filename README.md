...
#### v1.8.3
1. 修复：`--gitignore` 选项对目录的硬过滤未正确生效的问题。被 `.gitignore` 忽略的目录（如 `output/`、`node_modules/`）现在会完全从目录树中移除，而不是仅隐藏其内容。
2. 修复：`.gitignore` 模式匹配时的 domain 参数错误，该错误导致根目录下的直接子目录无法被 `.gitignore` 规则正确匹配。

## 计划
1. 移除 -Fc/-fc 配置项，使用一种新的解析配置的方式来规定配置文件包括支持的内容，保留 --config 进行调用。
2. 拆分单文件设计，模块化各个部分，方便开发和维护。
3. 支持英文。
4. 编写测试逐步完成所有功能的基本测试，便于后续开发中对原有功能的非破坏更新。
   1. 保留目前test中的基本测试功能。
   2. 测试对软连接文件的打包和还原。